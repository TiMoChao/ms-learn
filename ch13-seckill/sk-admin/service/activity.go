...
func (p ActivityServiceImpl) syncToZk(activity *model.Activity) error {
	...
	//fmt.Println(secProductInfoList)
	//fmt.Println(err)
	//fmt.Println(6565656565656)
	...
	for _, v := range activityList {
		...
		err = gconv.Struct(v, ac)

		if err != nil {
			log.Printf("ActivityEntity.GetActivityList999, err : %v", err)
		}

		secProductInfo.EndTime = ac.EndTime
		secProductInfo.OnePersonBuyLimit = ac.BuyLimit
		secProductInfo.ProductId = ac.ProductId
		secProductInfo.SoldMaxLimit = ac.Speed
		secProductInfo.StartTime = ac.StartTime
		secProductInfo.Status = ac.Status
		secProductInfo.Total = ac.Total
		secProductInfo.BuyRate = ac.BuyRate
